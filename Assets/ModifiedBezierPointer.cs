...
//
// Modified by WW to set the distance based on the upward angle, instead of having a
// static distance variable. This makes it easier to navigate in some scenarios.
//====================================================================================

using UnityEngine;
using System.Collections;

public class ModifiedBezierPointer : SteamVR_WorldPointer {
    ...
    private float GetForwardBeamLength() {
        ...
        float halfLength = pointerLength / 2;
        if (transform.forward.y < 0) {
            // if they're pointing downward then go to max length / 2
            actualLength = halfLength;
        } else {
            // otherwise use maximum length if they are pointing at 45 degrees, otherwise decrease linearly
            var angle = Vector3.Angle(transform.forward, transform.forward + (Vector3.down * transform.forward.y));
            if (angle > 90)
                angle -= 90;
            if (angle < 45)
                actualLength = halfLength + (angle / 45.0f) * halfLength;
            else
                actualLength = ((90.0f - angle) / 45.0f) * pointerLength;
        }
        ...
        if (hasRayHit && pointerContactDistance < actualLength) {
